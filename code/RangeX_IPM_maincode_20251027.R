...
# check how much the bootstrapped lambdas are skewed for all species (species with skewed lambdas: salpra vege, brapin vege, daucar bare ambi lo)
...
# plot lambdas with 0.025 and 0.975 as error bars
ggplot(data = lambda.allcombis.bootpara_c1, aes(x = as.factor(site_treat_combi), y = lambda, col = site)) +
  geom_point(aes(shape = trend), size = 3) +
  ...
  ggtitle("Parametric bootstrapping (CI is quantiles)")
...
png("/Users/eviseli/Desktop/FigPGR_hiambi_20251001_census1.png", width = 17, height = 10, units="cm", res=800)
...
png("/Users/eviseli/Desktop/FigPGR_hiambi_20251001_census2.png", width = 17, height = 10, units="cm", res=800)
...
#png("/Users/eviseli/Desktop/FigPGR_hiwarm_20250107.png", width = 17, height = 10, units="cm", res=800)
...
#png("/Users/eviseli/Desktop/FigPGR_loambi_20250107.png", width = 17, height = 10, units="cm", res=800)
...
png("/Users/eviseli/Desktop/FigPGR_combined_20251001_census1.png", width = 17, height = 17, units="cm", res=800)
...
png("/Users/eviseli/Desktop/FigPGR_combined_20251001_census2.png", width = 17, height = 17, units="cm", res=800)
...
png("/Users/eviseli/Desktop/FigPGR_ambivswarm_20251006_census1.png", width = 17, height = 10, units="cm", res=800)

ggplot(data = lambda.allcombis_c1_hi_widewarm ,
       aes(x = lambda.ambi, y = lambda.warm)) +
  geom_ribbon(data = band, inherit.aes = FALSE, aes(x = x, ymin = -Inf,            ymax = x * (1 - tol)), fill = "#5B7D6D", alpha = 0.1) +
  geom_ribbon(data = band, inherit.aes = FALSE, aes(x = x, ymin = x * (1 + tol),   ymax = Inf), fill = "#C17C0F", alpha = 0.1) +
...
plot_comp_effect <- ggplot() +
  ...
  theme(
    ...
    #legend.title = element_blank(),
    strip.background = element_blank(),
    #legend.position = c(0.75, 0.65),  # 0.95
...
png("/Users/eviseli/Desktop/FigPGR_barevsvege_20251006_census1.png", width = 10, height = 10, units="cm", res=800)
...
### OLD NONBOOTSTRAPPING APPROACH ###


# calculate bias corrected confidence intervals of competition-treatment difference of "real" lambdas
alpha <- 0.05

lambda.allcombis_c1_diffcomp.bcpi <- lambda_bootpara_c1_diffcomp %>%
  group_by(species, site, treat_warm) %>%
  summarize(bootstrapped_values_delta = list(delta_lambda), 
            bootstrapped_values_lrr = list(lrr_lambda),
            bootstrapped_values_pct = list(pct_change_rel_bare),
            .groups = 'drop') %>%
  ungroup() %>%
  left_join(lambda.allcombis_c1_diffcomp, by = c("species", "site", "treat_warm")) %>%
  mutate(bcpi_CI_delta = map2(bootstrapped_values_delta, delta_lambda, ~ bcpi(.y, .x, alpha)),  # .y is real_lambda, .x is t (bootstrapped values)
         lower_BCPI_delta = sapply(bcpi_CI_delta, `[[`, 1),  
         upper_BCPI_delta = sapply(bcpi_CI_delta, `[[`, 2),
         mean_bootstrap_delta = sapply(bootstrapped_values_delta, mean)) %>%
  mutate(bcpi_CI_lrr = map2(bootstrapped_values_lrr, lrr_lambda, ~ bcpi(.y, .x, alpha)),  # .y is real_lambda, .x is t (bootstrapped values)
         lower_BCPI_lrr = sapply(bcpi_CI_lrr, `[[`, 1),  
         upper_BCPI_lrr = sapply(bcpi_CI_lrr, `[[`, 2),
         mean_bootstrap_lrr = sapply(bootstrapped_values_lrr, mean)) %>%
  mutate(bcpi_CI_pct = map2(bootstrapped_values_pct, pct_change_rel_bare, ~ bcpi(.y, .x, alpha)),  # .y is real_lambda, .x is t (bootstrapped values)
         lower_BCPI_pct = sapply(bcpi_CI_pct, `[[`, 1),  
         upper_BCPI_pct = sapply(bcpi_CI_pct, `[[`, 2),
         mean_bootstrap_pct = sapply(bootstrapped_values_pct, mean)) %>%
  mutate(site_comp_warm = paste(site, "bare", treat_warm, sep = "."),
         site_warm = paste(site, treat_warm, sep = "_")) %>%
  ungroup() 

# adapt same order as for other lambda plots
sp_order <- c("daucar", "medlup", "cenjac", "hypper", "plamed", "broere", "salpra", "brapin", "silvul",  "scacol")
lambda.allcombis_c1_diffcomp.bcpi$species <- factor(lambda.allcombis_c1_diffcomp.bcpi$species, levels = sp_order)
...
# define position_dodge for three elements per group
posd_three <- position_dodge(width = 0.85) # position_dodge2(width = 0.7, preserve = "single")

# plot LRR to show effect of competition

png("/Users/eviseli/Desktop/FigPGR_barevsvege_20251006_census1.png", width = 19, height = 10, units="cm", res=800)

ggplot(data = lambda.allcombis_c1_diffcomp.bcpi, aes(x = species, y = lrr_lambda, col = site_warm)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = band_lo, ymax = band_hi,
           fill = "grey80", alpha = 0.25) +
  # reference lines
  geom_hline(yintercept = c(lrr_zero, lrr_double, lrr_half),
             linetype = c("dashed","dotted","dotted"), linewidth = c(.5,.4,.4),
             color = "grey40") +
  geom_errorbar(aes(ymin = lower_BCPI_lrr, ymax = upper_BCPI_lrr, group = site_comp_warm),  position = posd_three, width = 0.2, alpha = 1) +
  geom_point(aes(group = site_comp_warm), position = posd_three, shape = 21, size = 6,
             fill = "white", colour = "white", alpha = 1, stroke = 0) +
  geom_point(aes(group = site_comp_warm), position = posd_three, size = 4, fill = "white", stroke = 1.5) +
  theme_bw() +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 14),
    ...
    legend.title = element_blank(),
    strip.background = element_blank(),
    legend.position = c(0.82, 0.65),  # 0.95
    legend.justification = c(1, 0),
    legend.box = "vertical",  # combine legends vertically into a single box
    legend.box.background = element_rect(fill = "white", color = "black"),
    #legend.key.size = unit(1.5, "lines"),
  ...
  scale_color_manual(values = treat_warm_site_col, 
                     name = "Site & warming treatment", 
                     labels = c("lo_ambi" = "within-range, ambient", "hi_ambi" = "beyond-range, ambient", "hi_warm" = "beyond-range, warmed"),
                     guide = guide_legend(order = 1))
...
### get elasticities for seed requirement --------------------------------------
#
## choose which parameters to test (present in your 'params')
#par_names_to_test <- c(
#  "surv.int","surv.slope",
#  "growth.int","growth.slope","growth.sigma",
#  "flow.int","flow.slope",
#  "seed.int","seed.slope",
#  "germ.int","est.int",
#  "r_mean","r_sd")
#
## sst_combinations, params_list ans n_mesh are already defined
#
## compute elasticities
#elasticity_table <- map_dfr(names(params_list), function(key) {
#  par <- params_list[[key]]
#  
#  # only test parameters that exist and are not NA
#  valid_pars <- par_names_to_test[par_names_to_test %in% names(par) &
#                                    !is.na(unlist(par[par_names_to_test]))]
#  
#  els <- map_dfr(valid_pars, ~ elasticity_required_seeds(
#    par = par, par_name = .x, n_mesh = n_mesh,
#    target_pop_size = 5000, nstep = 10, sc1_index = 1, step = 0.01))
#  
#  els %>% mutate(species_site_treat = key)
#})
#
## select only combinations with lambda > 1 at hi ambi vege
#combis_of_interest <- c("daucar_hi_vege.ambi","medlup_hi_vege.ambi","cenjac_hi_vege.ambi","hypper_hi_vege.ambi","plamed_hi_vege.ambi","broere_hi_vege.ambi")
#
## elasticities for combis of interest, ranked (largest positive = biggest bottleneck)
#elasticity_ofinterest <- elasticity_table %>%
#  filter(species_site_treat %in% combis_of_interest) %>%
#  group_by(species_site_treat) %>%
#  arrange(desc(elasticity), .by_group = TRUE) %>%
...
# "On the log (additive) scale, about 7 % of the difference in required seeds between P. media and 
# D. carota is due to germination + establishment, and 93 % is due to post-establishment dynamics."
...
# calculate required starting seeds
res_list <- vector("list", nrow(bootpara_wide))

for (i in 1:nrow(bootpara_wide)) {
  
  par <- params_list_bootpara[[i]]  # get parameters for the specific combination
  key <- names(params_list_bootpara)[i]
  
  # generate the kernel with an appropriate number of mesh points (n)
  n_meshpoints <- 500 
  ipm <- mk.kernel(L = as.numeric(par$L), U = as.numeric(par$U), par = par, n = n_meshpoints, ceiling_growth = TRUE, ceiling_seeds = FALSE)
...
required_seeds_sc1_params_boot <- do.call(rbind, res_list)


write.csv(required_seeds_sc1_params_boot, "/Users/eviseli/Documents/GitHub/RangeX_IPMs/output/most_recent/required_seeds_sc1_params_boot.csv")


mean(required_seeds_sc1_params_boot[grepl("brapin_lo_bare.ambi", required_seeds_sc1_params_boot$species_site_treat) == TRUE,]$required_sc1, na.rm = TRUE)
mean(popsize_bootpara[grepl("brapin_lo_bare.ambi", popsize_bootpara$parameter_name) == TRUE,]$n0_sc1, na.rm = TRUE)
...
spec_startposize_nobro <- c("cenja", "daucar", "hypper", "medlup", "plamed")
...
required_seeds_sc1_params_boot <- required_seeds_sc1_params_boot %>%
  mutate(species = str_sub(species_site_treat, 1, 6),
         site = str_sub(species_site_treat, 8, 9),
         treat_combi = str_sub(species_site_treat, 11, 19)) 
...
min(popsize_bootpara_selected$required_sc1) # nope
...
popsize.allcombis.bootpara <- popsize_bootpara_selected %>%
  filter(is.na(required_seeds) == FALSE) %>% # get rid of NA's (the bootstrapped lambdas are not all >= 1 just because the "real" value is)
...
# plot
#png("/Users/eviseli/Desktop/FigSPS_hiambi_20250122.png", width = 17, height = 10, units="cm", res=800)
...
# looks the same, but numbers vary
...
# get the starting pop. size data ready, exclude all NAs
popsize_bootpara <- lambda_popsize_bootpara_c1 %>%
...
# quick plot to look at distribution 

png("/Users/eviseli/Desktop/sc1_vs_sc10_log_20250122.png", width = 17, height = 17, units="cm", res=800)

ggplot(data = popsize_bootpara) +
  geom_jitter(size = 3, aes(x = as.factor(site_treat_combi), y = log(n0_sc1)), col = "darkgreen", alpha = 0.5) +
  geom_jitter(size = 3, aes(x = as.factor(site_treat_combi), y = log(n0_sc10)), col = "darkblue", alpha = 0.5) +
  facet_wrap(~species, scales = "free") + # , scales = "free"
  geom_abline(intercept = 1, slope = 0) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "site - treatment combinations", y = "lambda") +
  ggtitle("Parametric bootstrapping of pop. size (CI is quantiles)") +
  geom_point(data = required_n0_df_long, aes(x = as.factor(site_treat_combi), y = log(required_n0)), col = "black")

dev.off()

png("/Users/eviseli/Desktop/sc1_vs_sc10_20250122.png", width = 17, height = 17, units="cm", res=800)

# plot all bootstrapped values (not logged)
ggplot(data = popsize_bootpara) +
  geom_jitter(size = 3, aes(x = as.factor(site_treat_combi), y = n0_sc1), col = "darkgreen", alpha = 0.5) +
  geom_jitter(size = 3, aes(x = as.factor(site_treat_combi), y = n0_sc10), col = "darkblue", alpha = 0.5) +
  facet_wrap(~species, scales = "free") + # , scales = "free"
  geom_abline(intercept = 1, slope = 0) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "site - treatment combinations", y = "lambda") +
  ggtitle("Parametric bootstrapping of pop. size (CI is quantiles)") +
  geom_point(data = required_n0_df_long, aes(x = as.factor(site_treat_combi), y = required_n0), col = "black")
...
# extract only hi site vege.ambi data and the species with PGR >= 1
popsize_bootpara_selected <- popsize_bootpara %>%
  filter(site == "hi" & treat_combi == "vege.ambi" & species %in% spec_startposize)

# are there negative values?
min(popsize_bootpara_selected$n0_sc1) # nope

# make a long data frame
popsize_bootpara_selected_long <- popsize_bootpara_selected %>%
  pivot_longer(cols = starts_with("n0_sc"), 
               names_to = "sc", 
               values_to = "n0") %>%
  mutate(sc = if_else(sc == "n0_sc1", "sc1", "sc10"))

# calculate the number of seeds rather than individuals in small size classes
popsize_bootpara_selected_long <- popsize_bootpara_selected_long %>%
  dplyr::select(-parameter_name, -site_treat_combi) %>%
  left_join(germ_est, by = c("site", "species", "treat_combi", "treat_comp", "treat_warm")) %>%
  mutate(n0_seeds = n0/prob_seed_est,
         n0_log = log(n0),
         n0_seeds_log = log(n0_seeds))


# calculate uncorrected confidence intervals of the raw values and the logged values
popsize.allcombis.bootpara <- popsize_bootpara_selected_long %>%
  filter(is.na(n0) == FALSE) %>% # get rid of NA's (the bootstrapped lambdas are not all >= 1 just because the "real" value is)
  group_by(site, species, treat_combi, treat_warm, treat_comp, sc) %>%
  summarize(boot_mean = mean(n0),
            lower_CI = quantile(n0, 0.025),
            upper_CI = quantile(n0, 0.975),
            lower_CI_log = quantile(n0_log, 0.025),
            upper_CI_log = quantile(n0_log, 0.975),
            boot_mean_seeds = mean(n0_seeds),
            lower_CI_seeds = quantile(n0_seeds, 0.025),
            upper_CI_seeds = quantile(n0_seeds, 0.975),
            lower_CI_seeds_log = quantile(n0_seeds_log, 0.025),
            upper_CI_seeds_log = quantile(n0_seeds_log, 0.975)) %>%
  ungroup() %>%
  left_join(required_n0_df_long, by = c("site", "species", "treat_combi", "sc", "treat_comp", "treat_warm")) %>%
  mutate(site_treat_combi = paste(site, treat_combi, sep = "_"),
         required_n0_log = log(required_n0),
         required_n0_seeds_log = log(required_n0_seeds))


# calculate bias corrected confidence intervals of "real" starting pop sizes --> only for species for which their PGR > 1
alpha <- 0.05

popsize.allcombis.bootpara.bcpi <- popsize_bootpara_selected_long %>%
  filter(species %in% spec_startposize & is.na(n0) == FALSE) %>% # only keep species where the PGD >= 1
  group_by(species, site, treat_combi, treat_warm, treat_comp, sc) %>%
  summarize(bootstrapped_values = list(n0), 
            bootstrapped_values_seeds = list(n0_seeds),
            bootstrapped_values_log = list(n0_log), 
            bootstrapped_values_seeds_log = list(n0_seeds_log),
            .groups = 'drop') %>%
  ungroup() %>%
  left_join(required_n0_df_long, by = c("species", "site", "treat_combi", "sc", "treat_comp", "treat_warm")) %>%
  mutate(bcpi_CI = map2(required_n0, bootstrapped_values, ~ bcpi(.x, .y, alpha)),  # .x = required_n0, .y = bootstrapped_values
         lower_BCPI = sapply(bcpi_CI, `[[`, 1),  
         upper_BCPI = sapply(bcpi_CI, `[[`, 2),
         mean_bootstrap = sapply(bootstrapped_values, mean),
         bcpi_CI_seeds = map2(required_n0_seeds, bootstrapped_values_seeds, ~ bcpi(.x, .y, alpha)),  # .x = required_n0, .y = bootstrapped_values
         lower_BCPI_seeds = sapply(bcpi_CI_seeds, `[[`, 1),  
         upper_BCPI_seeds = sapply(bcpi_CI_seeds, `[[`, 2),
         mean_bootstrap_seeds = sapply(bootstrapped_values_seeds, mean),
         bcpi_CI_log = map2(required_n0_log, bootstrapped_values_log, ~ bcpi(.x, .y, alpha)),  # .x = required_n0, .y = bootstrapped_values
         lower_BCPI_log = sapply(bcpi_CI_log, `[[`, 1),  
         upper_BCPI_log = sapply(bcpi_CI_log, `[[`, 2),
         mean_bootstrap_log = sapply(bootstrapped_values_log, mean),
         bcpi_CI_seeds_log = map2(required_n0_seeds_log, bootstrapped_values_seeds_log, ~ bcpi(.x, .y, alpha)),  # .x = required_n0, .y = bootstrapped_values
         lower_BCPI_seeds_log = sapply(bcpi_CI_seeds_log, `[[`, 1),  
         upper_BCPI_seeds_log = sapply(bcpi_CI_seeds_log, `[[`, 2),
         mean_bootstrap_seeds_log = sapply(bootstrapped_values_seeds_log, mean)) %>%
  ungroup() %>%
  mutate(treat_warm = str_sub(treat_combi, 6, 9),
         treat_comp = str_sub(treat_combi, 1,4),
         treat_warm_site = paste(site, treat_warm, sep = "_"), 
...
# get mean and sd across species
mean(popsize.allcombis.bootpara.bcpi[popsize.allcombis.bootpara.bcpi$sc == "sc1",]$required_n0_seeds)
sd(popsize.allcombis.bootpara.bcpi[popsize.allcombis.bootpara.bcpi$sc == "sc1",]$required_n0_seeds)



# make pretty (seeds no log)

ggplot(data = popsize.allcombis.bootpara.bcpi[popsize.allcombis.bootpara.bcpi$sc == "sc1",], 
       aes(x = species, y = required_n0_seeds)) +
  scale_x_discrete(labels = function(x) species_names[x]) +
  geom_jitter(data = popsize_bootpara_selected_long[popsize_bootpara_selected_long$sc == "sc1",], 
              aes(x = species, y = n0_seeds), col = "lightgray", width = 0.2) +
  geom_point(position = position_dodge(width = 0.7)) +
  geom_errorbar(aes(ymin = lower_BCPI_seeds, ymax = upper_BCPI_seeds), width = 0.2, position = position_dodge(width = 0.7)) +
  theme_bw() +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 14),
  ...
  labs(y = "Starting population size") +
  facet_wrap(~ species, scales = "free")
...
plot_startpop <- ggplot(data = popsize.allcombis.bootpara.bcpi[popsize.allcombis.bootpara.bcpi$sc == "sc1",], 
         aes(x = species, y = required_n0_seeds)) +
  scale_x_discrete(labels = function(x) species_names[x]) +
  geom_jitter(data = popsize_bootpara_selected_long[popsize_bootpara_selected_long$sc == "sc1",], 
              aes(x = species, y = n0_seeds), col = "lightgray", width = 0.3) +
  geom_point(position = position_dodge(width = 0.7), col = "#5B7D6D", size = 3) +
  geom_errorbar(aes(ymin = lower_BCPI_seeds, ymax = upper_BCPI_seeds), width = 0.2, position = position_dodge(width = 0.7), col = "#5B7D6D") +
  theme_bw() +
  theme(
...
png("/Users/eviseli/Desktop/FigSPS_seeds_202502062.png", width = 8.5, height = 10, units="cm", res=800)

plot_startpop 

dev.off()

# plot competition effect and seed production next to each other

png("/Users/eviseli/Desktop/FigPopSizeCompEffect_combined_20251015_census2.png", width = 17, height = 10, units="cm", res=800)
...
#### check out the kernels which produce negative number of seeds ------------

# this code can be found in RangeX_IPM_FirstFit_20250924.R

#### check out the weird dividsion in broere start pop. sizes ----------------

# get bootpara wide coefficients data for broere hi to check out weird separation
popsize_bootpara_broere_hi <- popsize_bootpara %>%
  filter(species == "broere" & site == "hi") %>%
  mutate(n0_log = n0_sc1,
         division = if_else(n0_log > 18, "upper", "lower")) %>%
  left_join(lambda_bootpara_c1, by = c("parameter_name", "treat_combi", "treat_comp", "treat_warm", "bootstrap", "site", "species"))


bootpara_wide_broere_hi <- popsize_bootpara_broere_hi %>%
  left_join(bootpara_wide, by = c("site", "species", "treat_combi", "bootstrap"))

# for plotting
bootpara_wide_broere_hi_long <- bootpara_wide_broere_hi %>%
  pivot_longer(cols = 13:28, # L to intercept_germ_rate
               names_to = "parameter", 
               values_to = "value")

# plot the different vital rates and colour after division
ggplot(data = bootpara_wide_broere_hi_long, aes(x = parameter, y = value, col = division)) +
  geom_jitter() +
  facet_wrap(site ~ treat_combi, scales = "free") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) # survival coefficients unstable for broere hi


# ok, so survival for broere and salpra must somehow be fixed --> check out raw survival data

# get rid of NAs
dat_surv_check <- dat_YS_ready %>%
  mutate(treat_warm = str_sub(unique_plant_ID, 8, 11),
         treat_comp = str_sub(unique_plant_ID, 13, 16),
         site = str_sub(unique_plant_ID, 5, 6),
         treat_combi = paste(treat_comp, treat_warm, sep = "."),
         site_treat_combi = paste(site, treat_combi, sep = "_")) %>%
  ungroup() #%>%
  #filter(is.na(survival) == FALSE & is.na(size_t0_log) == FALSE)

ggplot(data = dat_surv_check, aes(x = size_t0_log, y = survival, col = survival)) +
  geom_point() +
  facet_grid(species ~ site_treat_combi)
...
# broere at hi site has very few deaths


### supplementation numbers ----------------------------------------------------

## how many plants would need to be supplemented every year to keep the population going?
#supplementation_real <- lambda.allcombis_c1 %>%
#  left_join(
#    vr_wide %>% 
#      select(site, species, treat_combi, germ.int, est.int),
#    by = c("site", "species", "treat_combi")) %>%
#  mutate(est.rate = 1 / (1 + exp(-est.int)),  # transform intercept of logistic model into establishment probability using logistic transformation
#         prob_seed_est = germ.int * est.rate,      # combine germination and establishment
#         sup_adult = if_else(lambda < 1, 100 * (1 - lambda), 0),  # calculate number of adults which need to be supplemented
#         sup_seeds = if_else(lambda < 1, sup_adult / prob_seed_est, 0))  # calculate seeds needed
#
#
#
## do the same for all bootstrapped lambdas and vital rates
#supplementation_bootstrap <- lambda_bootpara %>%
#  mutate(bootstrap = as.integer(bootstrap)) %>%
#  left_join( # add bootpara_wide on to get germ.int and est.int
#    bootpara_wide %>% 
#      select(site, species, treat_combi, bootstrap, intercept_germ_rate, intercept_establishment),
#    by = c("site", "species", "treat_combi", "bootstrap")) %>%
#  mutate(est_rate = 1 / (1 + exp(-intercept_establishment)),  # transform establishment logit
#         prob_seed_est = intercept_germ_rate * est_rate,      # combine germination and establishment
#         sup_adult = if_else(lambda < 1, 100 * (1 - lambda), 0),  # calculate number of adults which need to be supplemented
#         sup_seeds = if_else(lambda < 1, sup_adult / prob_seed_est, 0))  # calculate seeds needed
#
#
## now plot!
#
#pdf("/Users/eviseli/Desktop/supplementation_numbers_cutoff.pdf")
#
#a <- ggplot(data = supplementation_bootstrap, aes(x = species, y = sup_seeds, col = species)) +
#  geom_point() +
#  facet_grid(site~treat_combi, scales = "free") +
#  ylim(- 10000, 10000) +
#  theme_bw() +
#  theme(axis.text.x = element_text(angle = 90))
#
...
#dev.off()
#
#pdf("/Users/eviseli/Desktop/supplementation_numbers_adult_cutoff.pdf")
#
#a <- ggplot(data = supplementation_bootstrap, aes(x = species, y = sup_adult, col = species)) +
#  geom_point() +
#  facet_grid(site~treat_combi, scales = "free") +
#  #ylim(- 10000, 10000) +
#  theme_bw() +
...
#dev.off()
#
#pdf("/Users/eviseli/Desktop/supplementation_numbers_boxplot.pdf")
#
#a <- ggplot(data = supplementation_bootstrap, aes(x = species, y = sup_seeds, col = species)) +
#  geom_boxplot() +
#  facet_grid(site~treat_combi, scales = "free") +
...
#dev.off()
#
## count how many lambdas are below 1 in the bootstraps
#shrink_pop <- supplementation_bootstrap %>%
#  group_by(site, species, treat_combi) %>%
#  summarize(no_below_1 = sum(lambda < 1, na.rm = TRUE))

# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# SEEDILING ESTABLISHMENT PROBABILITY  ----
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# plot germination rate based on vital rate models

# gray points: germination rate * establishment rate (as observed in the field)
# green points & error bars: germ. probability * est. probability (as predicted by the vital rate models)

# create a nice data frame
dat_germ_ent <- pred_germ %>%
  dplyr::select(germination_position_ID, site, species, treat_combi, exp, germ_rate, germ.prob) %>%
  left_join(pred_est %>%
              dplyr::select(germination_position_ID, site, species, treat_combi, exp, est.prob), 
            by = c("germination_position_ID", "site", "species", "treat_combi", "exp")) %>%
  mutate(germ_est_sticks = germ_rate * est.prob,
         germ_est_tot = germ.prob * est.prob) %>%
  filter(site == "hi" & treat_combi == "vege.ambi" & species %in% spec_startposize)

# calculate uncorrected confidence intervals of the raw values and the logged values
dat_germ_ent.ci <- dat_germ_ent %>%
  group_by(species, site, treat_combi, germ_est_tot) %>%
  summarize(sticks_mean = mean(germ_est_sticks),
            lower_CI = quantile(germ_est_sticks, 0.025),
            upper_CI = quantile(germ_est_sticks, 0.975),
            stdev = sd(germ_est_sticks),
            upper_sd = sticks_mean + stdev,
            lower_sd = sticks_mean - stdev)

# calculate bias corrected confidence intervals of "real" starting pop sizes --> only for species for which their PGR > 1
alpha <- 0.05

dat_germ_ent.bcpi <- dat_germ_ent %>%
  group_by(species, site, treat_combi, germ_est_tot) %>%
  summarize(germ_est_sticks_vector = list(germ_est_sticks), 
            .groups = 'drop') %>%
  ungroup() %>%
  mutate(bcpi_CI = map2(germ_est_tot, germ_est_sticks_vector, ~ bcpi(.x, .y, alpha)),  # .x = required_n0, .y = bootstrapped_values
         lower_BCPI = sapply(bcpi_CI, `[[`, 1),  
         upper_BCPI = sapply(bcpi_CI, `[[`, 2),
         mean_vector = sapply(germ_est_sticks_vector, mean)) %>%
  ungroup() 

# get mean and sd
mean(dat_germ_ent.bcpi$germ_est_tot)
sd(dat_germ_ent.bcpi$germ_est_tot)
...
# re-order species
dat_germ_ent.ci$species <- factor(dat_germ_ent.ci$species, levels = spec_startposize)
dat_germ_ent$species <- factor(dat_germ_ent$species, levels = spec_startposize)


plot_estrate <- ggplot() +
  geom_jitter(data = dat_germ_ent, aes(x = species, y = germ_est_sticks), col = "lightgray", width = 0.3) +
  geom_point(data = dat_germ_ent.ci, aes(x = species, y = germ_est_tot), size = 3, col = "#3b5549") +
  geom_errorbar(data = dat_germ_ent.ci, aes(x = species, ymin = lower_sd, ymax = upper_sd), width = 0.2, col = "#3b5549") +
  #geom_boxplot(data = dat_germ_ent, aes(x = species, y = germ_est_sticks))
  theme_bw() +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 14),
    ...
    #legend.key.width = unit(1.5, "lines"),
    legend.spacing = unit(-0.5, "lines")) +
  labs(y = "Seedling establishment probability") +
  scale_x_discrete(labels = function(x) species_names[x]) 


# plot and save together

png("/Users/eviseli/Desktop/FigSPS_combined_20250122.png", width = 17, height = 10, units="cm", res=800)
...
avg_seed <- pred_seed %>%
  filter(site == "lo" & treat_comp.x == "vege" & species %in% spec_startposize) %>% # site == "lo" & 
  group_by(species) %>%
  summarize(mean_seedno = mean(number_seeds, na.rm = TRUE),
            sd_seedno = sd(number_seeds, na.rm = TRUE), # /sqrt(length(pred_seed[is.na(pred_seed$number_seeds) == FALSE,]))
...
ggplot() +
  geom_jitter(data = pred_seed[pred_seed$site == "lo" & pred_seed$treat_comp.x == "vege" & pred_seed$species %in% spec_startposize,], aes(x = species, y = number_seeds), col = "lightgray", width = 0.3) +
  geom_point(data = avg_seed, aes(x = species, y = mean_seedno), size = 3, col = "#3b5549") +
  geom_errorbar(data = avg_seed, aes(x = species, ymin = lower_sd, ymax = upper_sd), width = 0.2, col = "#3b5549") +
  #geom_boxplot(data = dat_germ_ent, aes(x = species, y = germ_est_sticks))
  theme_bw() +
  theme(
    axis.title.x = element_blank(),
    ...
    #legend.key.width = unit(1.5, "lines"),
    legend.spacing = unit(-0.5, "lines")) +
  labs(y = "Mean seed production (no. seeds)") +
  scale_x_discrete(labels = function(x) species_names[x]) +
  scale_y_log10(
    limits = c(0.00001, 1000000000000000),
    breaks = scales::trans_breaks("log10", function(x) 10^x),
    labels = scales::trans_format("log10", scales::math_format(10^.x))) 
...
## get data for ambi vege at low site (this is where seeds would naturally be released)
#seed_release <- dat_YS_raw %>%
#  filter(grepl("lo.ambi.vege", unique_plant_ID) == TRUE & is.na(height_reproductive_str) == FALSE)
#
#t.test(seed_release[seed_release$species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$height_reproductive_str,
#       seed_release[seed_release$species %nin% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$height_reproductive_str)
#
#ggplot(data = seed_release, aes(x = species, y = height_reproductive_str)) +
#  geom_boxplot()

# this would need "real" height, not stretched height, which wasn't recorded...

### SEED SIZE

# get data for ambi vege at low site (this is where seeds would naturally be released)
seeds_lo.vege <- seeds %>%
  filter(grepl("lo.ambi.vege", unique_plant_ID) == TRUE)

t.test(seeds_lo.vege[seeds_lo.vege$species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight,
       seeds_lo.vege[seeds_lo.vege$species %nin% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight)

ggplot(data = seeds_lo.vege, aes(x = species, y = seedweight)) +
  geom_boxplot()

# what if just seedweight in general, not only at lo.vege? --> better, it's about species level differences

t.test(seeds[seeds$species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight,
       seeds[seeds$species %nin% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight)

mean(seeds[seeds$species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight, na.rm = TRUE)
sd(seeds[seeds$species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight, na.rm = TRUE)

mean(seeds[seeds$species %nin% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight, na.rm = TRUE)
sd(seeds[seeds$species %nin% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"),]$seedweight, na.rm = TRUE)


ggplot(data = seeds, aes(x = species, y = seedweight)) +
  geom_boxplot()

# calculate uncorrected confidence intervals of the raw values
seeds <- seeds %>%
  mutate(group = if_else(species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"), "up", "down"))

seeds.ci <- seeds %>%
  filter(is.na(seedweight) == FALSE) %>%
  group_by(species) %>%
  summarize(mean_seedweight = mean(seedweight),
            lower_CI = quantile(seedweight, 0.025),
            upper_CI = quantile(seedweight, 0.975),
            stdev = sd(seedweight),
            upper_sd = mean_seedweight + stdev,
            lower_sd = mean_seedweight - stdev) %>%
  mutate(group = if_else(species %in% c("daucar", "medlup", "cenjac", "broere", "hypper", "plamed"), "up", "down"))

# plot

# re-order species
seeds.ci$species <- factor(seeds.ci$species, levels = sp_order)
seeds$species <- factor(seeds$species, levels = sp_order)

png("/Users/eviseli/Desktop/FigSW_20250130.png", width = 17, height = 10, units="cm", res=800)

ggplot() +
  geom_jitter(data = seeds, aes(x = species, y = seedweight), col = "lightgray", width = 0.3) +
  geom_point(data = seeds.ci, aes(x = species, y = mean_seedweight), size = 3, col = "#4D531B") +
  geom_errorbar(data = seeds.ci, aes(x = species, ymin = lower_sd, ymax = upper_sd), width = 0.2, col = "#4D531B") +
  #geom_boxplot(data = dat_germ_ent, aes(x = species, y = germ_est_sticks))
  theme_bw() +
  theme(
    axis.title.x = element_blank(),
    ...
    #legend.key.width = unit(1.5, "lines"),
    legend.spacing = unit(-0.5, "lines")) +
  labs(y = "Seedling weight (g)") +
  scale_x_discrete(labels = function(x) species_names[x]) 

dev.off()

png("/Users/eviseli/Desktop/FigSW_boxplot_20250130.png", width = 10, height = 10, units="cm", res=800)

ggplot(data = seeds, aes(x = group, y = seedweight)) +
  geom_jitter(col = "lightgray") +
  ...
  theme(
    ...
    legend.spacing = unit(-0.5, "lines")) +
  labs(y = "Seedling weight (g)") +
  scale_x_discrete(limits = c("up", "down"), labels = c("up" = "Population\n growth rate > 1", "down" = "Population\n growth rate < 1")) 
...
# do LTRE following Hanna's code in Nomoto & Alexander (2021)

# these are the steps to follow
# 1) for all vital rates, select 1 treatment as control or take the mean across some treatments to create a reference
# 2) calculate the midpoints of all vital rates between the control/ reference and the actual vital rates (for U and L: take the max/ min of both control and treatment based values)
# 3) create a full kernel with the midpoints and calculate a midpoint-lambda (needed later)
# 4) calculate the sensitivities of the mid-kernel for each vital rate with (perturbation of 0.001)
# 5) calculate the LTRE contributions for each vital rate, using: sensitivity for each vital rate, actual vital rate parameters, control/ reference vital rates


### control --------------------------------------------------------------------

# "reference" of all treatments: take the mean across all species * site combinations for all vital rate 
vr_ltre_reference <- vr_long %>%
  dplyr::select(-treat_warm, -treat_comp) %>%
  group_by(site, species, vital_rate) %>%
  summarize_at(vars(slope:U), mean, na.rm = TRUE) %>%
  mutate_at(vars(slope:U), ~ifelse(is.nan(.), NA, .)) %>%
  mutate(treat_combi = "reference") %>%
  ungroup()

# real control: use bare lo as control (intrinsic growth rate at lower elevation)
vr_ltre_control <- vr_long %>%
  filter(site == "lo" & treat_combi == "bare.ambi") %>%
  dplyr::select(-c(treat_warm, treat_comp, treat_combi, N_size)) %>%
  mutate(treat_combi = "control")

# dublicate this for site hi, as the site lo values are control for both sites
vr_ltre_control_hi <- vr_ltre_control %>%
  mutate(site = "hi")
vr_ltre_control <- bind_rows(vr_ltre_control, vr_ltre_control_hi)


### midpoints ------------------------------------------------------------------

# midpoints can be either between the "reference" (i.e. a mean across treatments as in vr_ltre_reference) or a real control (here bare lo, vr_ltre_control)

# calculate midpoints
param_cols <- c("intercept","slope","sigma","r_mean","r_sd") # which parameters need midpoints?

# create an empty list to store the midpoints
midpoints_con <- list() # collect rows, bind at the end

# loop through each row in vr_long
for (i in 1:nrow(vr_long)) {
  
  # get species and site for the current row in vr_long
  species_curr <- vr_long$species[i]
  site_curr <- vr_long$site[i]
  treat_combi_curr <- vr_long$treat_combi[i]
  vital_rate_curr <- vr_long$vital_rate[i]
  
  # 1) pull treatment & control rows
  treatment_row <- vr_long %>%
    dplyr::filter(species == species_curr,
                  site == site_curr,
                  treat_combi == treat_combi_curr,
                  vital_rate == vital_rate_curr) %>%
    dplyr::select(any_of(c("species","site","treat_combi","vital_rate","L","U", param_cols)))
  control_row <- vr_ltre_control %>%
    dplyr::filter(species == species_curr,
                  site == site_curr,
                  treat_combi == "control",
                  vital_rate == vital_rate_curr) %>%
    dplyr::select(any_of(c("species","site","treat_combi","vital_rate","L","U", param_cols))) %>%
    dplyr::rename(treat_combi = treat_combi)  # keep name consistent
  
  # check whether this addresses exactly one row each
  if (nrow(treatment_row) != 1L || nrow(control_row) != 1L) next
  
  # 2) caclualte common bounds for LTRE pair (i.e. U and L need to be combined!)
  L_common <- min(treatment_row$L, control_row$L, na.rm = TRUE)
  U_common <- max(treatment_row$U, control_row$U, na.rm = TRUE)
    
  # 3) calculate midpoint parameters (elementwise mean) on the param columns
  tr_par <- treatment_row |> dplyr::select(all_of(param_cols))
  ct_par <- control_row   |> dplyr::select(all_of(param_cols))
  
  # keep numerics as numerics
  tr_par[] <- lapply(tr_par, as.numeric)
  ct_par[] <- lapply(ct_par, as.numeric)
  
  mid_par <- (tr_par + ct_par) / 2
  
  # 4) assemble a tidy row (treat’s key + midpoint params + common L/U)
  mid_row <- tibble::tibble(
    ...
    dplyr::bind_cols(mid_par)
  
  midpoints_con[[length(midpoints_con) + 1L]] <- mid_row # add to list
}

# convert the result into a data frame with proper column names
vr_ltre_midpoint_con <- dplyr::bind_rows(midpoints_con)


### midpoint kernel ------------------------------------------------------------

# now get IPM kernels with the midpoint parameters for each species * site combination to check whether they make sense

# unique combinations of species, site, and treatment
sst_combinations <- vr_ltre_midpoint_con %>%
  distinct(species, site, treat_combi)

### USING LO BARE AS CONTROL

# save all parameters for all combinations in a list
params_list_ltre_con <- mk.params.list(data_vrcols = vr_ltre_midpoint_con, data_sstcombi = sst_combinations)

# force clean keys: species_site_treat
names(params_list_ltre_con) <- vr_ltre_midpoint_con %>%
  dplyr::distinct(species, site, treat_combi) %>%
  dplyr::transmute(key = paste(species, site, treat_combi, sep = "_")) %>%
  dplyr::pull(key)

# generate the kernels and plot
# list to store kernel results (named)
ipm.midpoint_con <- vector("list", length(params_list_ltre_con))
names(ipm.midpoint_con) <- names(params_list_ltre_con)

# create a progress bar for kernel generation
pb2 <- progress_estimated(length(params_list_ltre_con))

# loop through the list of parameters to calculate IPM kernels and plot
for (name in names(params_list_ltre_con)) {
  ...
  # retrieve the current parameters, L and U from the list
  current_params <- params_list_ltre_con[[name]]
  L_current <- current_params$L
  U_current <- current_params$U
  
  # fit the kernel using the parameters
  ipm_kernel <- mk.kernel(n = 500, L = L_current, U = U_current, par = current_params, ceiling_growth = FALSE, ceiling_seeds = FALSE)
  
  # store the kernel and mesh points in the ipm.allcombis list
  ipm.midpoint_con[[name]] <- list(
    K = ipm_kernel$K,
    G = ipm_kernel$G,
    R = ipm_kernel$R,
    meshpts = exp(ipm_kernel$meshpts),
...
# calculate lambdas
lambda_results_midpoint_con <- lapply(names(ipm.midpoint_con), function(name) {
  lambda.iter(k = ipm.midpoint_con[[name]]$K)  # apply lambda.iter to each K matrix while using the names of the elements for clarity
})

# name the output list
names(lambda_results_midpoint_con) <- names(ipm.midpoint_con)

# create a data frame for plotting
lambda_values_midpoint_con <- sapply(lambda_results_midpoint_con, function(x) x$lambda)
names_list <- names(lambda_results_midpoint_con)
lambda.midpoint_con <- data.frame(site_species_treat = names_list, lambda = lambda_values_midpoint_con, stringsAsFactors = FALSE, row.names = NULL) %>%
  mutate(species = str_sub(site_species_treat, 1, 6),
         site = str_sub(site_species_treat, 8, 9),
         treat_combi = str_sub(site_species_treat, 11, 19),
         trend = ifelse(lambda < 1, "shrink", "grow"),
         site_treat = paste(site, treat_combi, sep = "."))

ggplot(data = lambda.midpoint_con, aes(x = as.factor(site_treat), y = lambda, col = site)) +
  geom_point(aes(shape = trend), size = 3) +
  facet_wrap(~species, scales = "free") + # , scales = "free"
  geom_abline(intercept = 1, slope = 0) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "site - teratment combinations", y = "lambda") +
  ggtitle("Midpoint-lambdas (control: lo bare)")
...
### USING LO BARE AS A CONTROL

## calculate the sensitivities of the mid-kernel for each vital rate with the central finite difference method

# define the perturbation 
perturbation <- 1e-4  # .Machine$double.eps^(1/3)

# create a list to store the results
sensitivity_results_cfd <- list()

pb <- progress_estimated(length(params_list_ltre_con))  # progress bar

# loop through each parameter set (i.e. each species * site * treatment combination)
for (combi in names(params_list_ltre_con)) {
  pb$tick()$print()   # update progress
  
  # extract parameters for the current combination
  mid_par <- params_list_ltre_con[[combi]]
  
  # apply the sensitivity analysis to the current parameters
  sens_result <- sensitivity.analysis.finitediff(
    params = mid_par,
    eps  = perturbation,
    ceil_growth = FALSE,
    ceil_seeds  = FALSE,
  ...
  # store the result in the list using the combination name as the combi
  sens_result$species_site_treatment <- combi
  sensitivity_results_cfd[[combi]] <- sens_result
}

# combine all results into one data frame
sensitivity_df_cfd <- dplyr::bind_rows(sensitivity_results_cfd) %>%
  dplyr::filter(!(parameter %in% c("L","U"))) %>%   # drop size bounds
  tidyr::separate(species_site_treatment,
                  into = c("species","site","treat_combi"), sep = "_")
...
# make vr_long and vr_ltre_reference properly long
vr_long_long_con <- vr_long %>%
  pivot_longer(cols = c(6:12), names_to = "parameter", values_to = "values_org") %>%
  mutate(vital_rate = case_when(vital_rate == "size_t1_log" ~ "growth",
                                vital_rate == "flower_status" ~ "flow",
                                vital_rate == "survival" ~ "surv",
                                vital_rate == "number_seeds" ~ "seed",
                                vital_rate == "germ_rate" ~ "germ", 
                                vital_rate == "establishment" ~ "est",
                                vital_rate == "recruit_size" ~ "rcsz",
                                .default = vital_rate),
         parameter = case_when(parameter == "intercept" ~ "int",
                               parameter == "slope" ~ "slope",
                               .default = parameter),
         parameter = paste(vital_rate, parameter, sep = ".")) %>%
  filter(parameter %in% c("growth.int", "growth.slope", "growth.sigma", "surv.int", "surv.slope", "flow.int", "flow.slope", "seed.int", "seed.slope", "rcsz.r_mean", "rcsz.r_sd", 
                          "germ.int", "est.int")) %>%
  mutate(parameter = case_when(parameter == "rcsz.r_mean" ~ "r_mean",
                               parameter == "rcsz.r_sd" ~ "r_sd",
                               .default = parameter)) 

vr_ltre_control_long <- vr_ltre_control %>%
  pivot_longer(cols = c(4:10), names_to = "parameter", values_to = "values_control") %>%
  mutate(vital_rate = case_when(vital_rate == "size_t1_log" ~ "growth",
                                vital_rate == "flower_status" ~ "flow",
                                vital_rate == "survival" ~ "surv",
                                vital_rate == "number_seeds" ~ "seed",
  ...
  filter(parameter %in% c("growth.int", "growth.slope", "growth.sigma", "surv.int", "surv.slope", "flow.int", "flow.slope", "seed.int", "seed.slope", "rcsz.r_mean", "rcsz.r_sd", 
                          "germ.int", "est.int")) %>%
  dplyr::select(-treat_combi, -U, -L) %>%
  mutate(parameter = case_when(parameter == "rcsz.r_mean" ~ "r_mean",
                               parameter == "rcsz.r_sd" ~ "r_sd",
                               .default = parameter)) 


# merge the control value to the original values and the sensitivities, calculate LTRE contributions
vr_ltre_all <- vr_long_long_con %>%
  left_join(vr_ltre_control_long, by = c("site", "species", "vital_rate", "parameter")) %>%
  left_join(sensitivity_df_cfd, by = c("site", "species", "parameter", "treat_combi")) %>%
  mutate(values_org     = ifelse(is.na(values_org),     0, values_org), # replace missing coeffs with 0 so absences contribute 0
         values_control = ifelse(is.na(values_control), 0, values_control),
         sensitivity    = ifelse(is.na(sensitivity),    0, sensitivity)) %>%
  mutate(contri = sensitivity * (values_org - values_control))

# vital-rate–level contributions
vr_ltre_contribution <- vr_ltre_all %>%
  group_by(species, site, treat_combi, vital_rate) %>%
  summarise(sum_contri = sum(contri, na.rm = TRUE), .groups = "drop") %>%
  mutate(site_treat = paste(site, treat_combi, sep = "."))

### differences in lambda ------------------------------------------------------

# helper to build K with consistent flags (same as midpoint code)
buildK <- function(par, n = 500) {
  mk.kernel(par$L, par$U, par, n = n,
            ceiling_growth = FALSE, ceiling_seeds = FALSE)$K
}

# helper to parse a "species_site_treat" key into parts
parse_key <- function(key){
  x <- strsplit(key, "_", fixed = TRUE)[[1]]
  list(species = x[1], site = x[2], treat = x[3])
}

# recompute direct lambda (treat vs global control lo_bare.ambi) on midpoint L/U
# for ALL keys that appear in params_list_ltre_con (your original midpoint list)
lambda_direct_common0 <- lapply(names(params_list_ltre_con), function(key_mid){
  
  # e.g., "scacol_hi_vege.warm"
  parts <- parse_key(key_mid)
  species <- parts$species; site <- parts$site; treat <- parts$treat
  
  # skip the global control itself (lo_bare.ambi)
  if (site == "lo" && treat == "bare.ambi") return(NULL)
  
  # treatment key is exactly the key_mid (original parameters from params_list)
  treat_key   <- paste(species, site, treat, sep = "_")
  # global control key: lo bare.ambi for the SAME species
  control_key <- paste(species, "lo", "bare.ambi", sep = "_")
  
  # both must exist in your original param list
  if(!(treat_key %in% names(params_list)) || !(control_key %in% names(params_list))) {
    return(NULL)
  }
  
  # midpoint params for this pair (to get common L/U)
  par_m <- params_list_ltre_con[[key_mid]]
  
  # ORIGINAL parameters for treat/control
  par_t <- params_list[[treat_key]]
  par_c <- params_list[[control_key]]
  
  # overwrite bounds with the midpoint common L/U
  par_t$L <- par_c$L <- par_m$L
  par_t$U <- par_c$U <- par_m$U
  
  # recompute lambdas on common bounds
  lam_t <- lambda.iter(buildK(par_t, n = 500))$lambda
  lam_c <- lambda.iter(buildK(par_c, n = 500))$lambda
  ...
  data.frame(
    ...
    lambda_treat_common   = lam_t,
    lambda_control_common = lam_c,
    delta_lambda_direct_common = lam_t - lam_c,   # treat − control
    stringsAsFactors = FALSE
  )
})

# bind to one table
lambda_direct_common0 <- dplyr::bind_rows(lambda_direct_common0) %>%
  # keep your original extraction for treat_combi / site_treat
  dplyr::mutate(
    treat_combi = stringr::str_sub(treat_key, 11, 19),  # matches your convention
    site_treat  = paste(site, treat_combi, sep = ".")
  )


# calculate difference in lambdas based on the recomputed lambdas (NOT lambda.allcombis)
lambda_diff <- lambda_direct_common0 %>%
  dplyr::select(species, site, treat_combi,
                delta_lambda_direct = delta_lambda_direct_common) %>%
  dplyr::mutate(site_treat = paste(site, treat_combi, sep = "."))


## get the differences in lambda
#lambda_diff <- lambda.allcombis %>%
#  filter(!(site == "lo" & treat_combi == "bare.ambi")) %>% # drop the control rows
#  left_join(lambda.allcombis %>% # join control lambda by species (lo bare.ambi used as control for BOTH sites)
#              filter(site == "lo", treat_combi == "bare.ambi") %>%
#              dplyr::select(species, lambda_control = lambda),
#            by = "species") %>%
#  mutate(delta_lambda_direct = as.numeric(lambda) - as.numeric(lambda_control)) %>% # treat − control
#  dplyr::select(species, site, treat_combi, delta_lambda_direct)%>%
#  mutate(site_treat = paste(site, treat_combi, sep = "."))

# sum up all vital rate contributions
ltre_sum <- vr_ltre_all %>%
  group_by(site, species, treat_combi, treat_warm, treat_comp) %>%
  summarize(delta_lambda_LTRE = sum(contri, na.rm = TRUE)) %>%
  ungroup()

# compare signs match:
ltre_check <- lambda_diff %>%
  left_join(ltre_sum, by = c("species","site","treat_combi")) %>%
  mutate(diff = delta_lambda_LTRE - delta_lambda_direct)

# flag any big discrepancies
bad <- ltre_check %>% filter(abs(diff) > 1e-2)

# plot 
ggplot() + 
  geom_bar(data = vr_ltre_contribution, aes(x = species, y = sum_contri, fill = vital_rate), position = "stack", stat = "identity") +
  geom_point(data = lambda_diff, aes(x = species, y = delta_lambda_direct)) +
  facet_wrap(~site_treat) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank()) +
  labs(y ="Vital rate contributions")


# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# LIFE TABLE RESPONSE EXPERIMENTS: MULTIPLE CONTROLS  ----
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# comparisons made & corresponding controls:
# 1) bare vs. vege for each lo, hi ambi and hi bare: competition effect --> bare lo/hi ambi/hi warm
# 2) warm vs. ambi for each hi bare and hi vege: warming effect --> hi bare/hi vege


### control --------------------------------------------------------------------

# define control
vr_ltre_control1 <- vr_long %>%
  dplyr::filter(grepl("^bare\\.(ambi|warm)$", treat_combi)) %>%
  dplyr::select(-c(treat_warm, treat_comp, N_size)) %>%   # keep treat_combi (bare.ambi / bare.warm)
  dplyr::mutate(ctrl_warm = sub("^[^.]+\\.", "", treat_combi), # 'ambi' or 'warm'
                treat_combi = "control")


### midpoints ------------------------------------------------------------------

# result: three data frames with midpoints

# define the three treatment data frames
# 1) keep only vege.<warm> rows from vr_long as “treatments” for this LTRE
vr_treat_vege1 <- vr_long %>%
  dplyr::filter(grepl("^vege\\.(ambi|warm)$", treat_combi)) %>%
  dplyr::mutate(treat_warm = sub("^[^.]+\\.", "", treat_combi))  # 'ambi'/'warm'
...
# create an empty list to store the midpoints
midpoints_con1 <- list() # collect rows, bind at the end

# calculate midpoints
# 1)
# loop through each row in vr_ltre_treat_1
for (i in 1:nrow(vr_ltre_treat_1)) {
  
  # get species and site for the current row in vr_ltre_treat_1
  species_curr <- vr_treat_vege1$species[i]
  site_curr    <- vr_treat_vege1$site[i]
  treat_combi_curr <- vr_treat_vege1$treat_combi[i]      # vege.ambi / vege.warm
  warm_curr    <- vr_treat_vege1$treat_warm[i]           # ambi / warm
  vital_rate_curr <- vr_treat_vege1$vital_rate[i]
  
  # 1) pull treatment & control rows
  treatment_row <- vr_treat_vege1 %>%
    dplyr::filter(species == species_curr,
                  site    == site_curr,
                  treat_combi == treat_combi_curr,
                  vital_rate  == vital_rate_curr) %>%
    dplyr::select(dplyr::any_of(c("species","site","treat_combi","vital_rate","L","U", param_cols)))
  control_row <- vr_ltre_control1 %>%
    dplyr::filter(species == species_curr,
                  site    == site_curr,
                  ctrl_warm == warm_curr,
                  vital_rate  == vital_rate_curr) %>%
    dplyr::select(dplyr::any_of(c("species","site","vital_rate","L","U", param_cols, "ctrl_warm"))) %>%
    dplyr::mutate(treat_combi = paste0("bare.", ctrl_warm)) %>%   # label for clarity
    dplyr::select(-ctrl_warm)
  ...
  mid_row <- tibble::tibble(
    species     = species_curr,
    site        = site_curr,
    treat_combi = treat_combi_curr,   # vege.<warm>
    vital_rate  = vital_rate_curr,
    L = L_common,
    U = U_common
  ) |>
  ...
  midpoints_con1[[length(midpoints_con1) + 1L]] <- mid_row # add to list
}

# convert the result into a data frame with proper column names
vr_ltre_midpoint_con1 <- dplyr::bind_rows(midpoints_con1)
...
# unique species × site × treat keys (vege.<warm> only)
sst_combinations1 <- vr_ltre_midpoint_con1 %>%
  dplyr::distinct(species, site, treat_combi)


# save all parameters for all combinations in a list
params_list_ltre_con1 <- mk.params.list(data_vrcols  = vr_ltre_midpoint_con1, data_sstcombi = sst_combinations1)

# force clean keys: species_site_treat
names(params_list_ltre_con1) <- vr_ltre_midpoint_con1 %>%
  dplyr::distinct(species, site, treat_combi) %>%
  dplyr::transmute(key = paste(species, site, treat_combi, sep = "_")) %>%
  dplyr::pull(key)
...
# generate the kernels and plot
# list to store kernel results (named)
ipm.midpoint_con1 <- vector("list", length(params_list_ltre_con1))
names(ipm.midpoint_con1) <- names(params_list_ltre_con1)

# create a progress bar for kernel generation
pb2 <- progress_estimated(length(params_list_ltre_con1))

# loop through the list of parameters to calculate IPM kernels and plot
for (name in names(params_list_ltre_con1)) {
  ...
  # retrieve the current parameters, L and U from the list
  current_params <- params_list_ltre_con1[[name]]
  L_current <- current_params$L
  U_current <- current_params$U
...
# calculate lambdas
lambda_results_midpoint_con1 <- lapply(names(ipm.midpoint_con1), function(name) {
  lambda.iter(k = ipm.midpoint_con1[[name]]$K)
})
names(lambda_results_midpoint_con1) <- names(ipm.midpoint_con1)

# create a data frame for plotting
lambda_values_midpoint_con1 <- sapply(lambda_results_midpoint_con1, function(x) x$lambda)
names_list1 <- names(lambda_results_midpoint_con1)
lambda.midpoint_con1 <- data.frame(site_species_treat = names_list1,
                                   lambda = lambda_values_midpoint_con1,
                                   stringsAsFactors = FALSE, row.names = NULL) %>%
  dplyr::mutate(species = stringr::str_sub(site_species_treat, 1, 6),
                site = stringr::str_sub(site_species_treat, 8, 9),
                treat_combi = stringr::str_sub(site_species_treat, 11, 19),
                trend = ifelse(lambda < 1, "shrink", "grow"),
                site_treat = paste(site, treat_combi, sep = "."))
...
# create a list to store the results
sensitivity_results_cfd1 <- list()

pb <- progress_estimated(length(params_list_ltre_con1))  # progress bar

# loop through each parameter set (i.e. each species * site * treatment combination)
for (combi in names(params_list_ltre_con1)) {
  pb$tick()$print()
  mid_par <- params_list_ltre_con1[[combi]]
  sens_result <- sensitivity.analysis.finitediff(
    params      = mid_par,
    eps         = perturbation,
    ceil_growth = FALSE,
  ...
  sens_result$species_site_treatment <- combi
  sensitivity_results_cfd1[[combi]] <- sens_result
}

# combine all results into one data frame
sensitivity_df_cfd1 <- dplyr::bind_rows(sensitivity_results_cfd1) %>%
  dplyr::filter(!(parameter %in% c("L","U"))) %>%
  tidyr::separate(species_site_treatment, into = c("species","site","treat_combi"), sep = "_")


### LTRE contribution ----------------------------------------------------------


# long tables: (1) treatment rows = vege.<warm> only; (2) controls = bare.<warm>
vr_long_long_con1 <- vr_long %>%
  dplyr::filter(grepl("^vege\\.(ambi|warm)$", treat_combi)) %>%
  tidyr::pivot_longer(cols = c(6:12), names_to = "parameter", values_to = "values_org") %>%
  dplyr::mutate(vital_rate = dplyr::case_when(
    vital_rate == "size_t1_log"  ~ "growth",
    vital_rate == "flower_status"~ "flow",
    vital_rate == "survival"     ~ "surv",
    vital_rate == "number_seeds" ~ "seed",
    vital_rate == "germ_rate"    ~ "germ", 
    ...
    parameter = dplyr::case_when(
      ...
      .default = parameter),
    parameter = paste(vital_rate, parameter, sep = ".")) %>%
  dplyr::filter(parameter %in% c("growth.int","growth.slope","growth.sigma",
                                 "surv.int","surv.slope",
                                 "flow.int","flow.slope",
                                 "seed.int","seed.slope",
                                 "rcsz.r_mean","rcsz.r_sd","germ.int","est.int")) %>%
  dplyr::mutate(parameter = dplyr::case_when(
    parameter == "rcsz.r_mean" ~ "r_mean",
    parameter == "rcsz.r_sd"   ~ "r_sd",
    .default = parameter),
    warm = sub("^[^.]+\\.", "", treat_combi))    # 'ambi'/'warm'

vr_ltre_control_long1 <- vr_ltre_control1 %>%
  # pivot only the parameter columns by name (robust to column order)
  tidyr::pivot_longer(cols = tidyselect::any_of(param_cols),
                      names_to = "parameter", values_to = "values_control") %>%
  dplyr::mutate(
    vital_rate = dplyr::case_when(
      vital_rate == "size_t1_log"   ~ "growth",
      vital_rate == "flower_status" ~ "flow",
      vital_rate == "survival"      ~ "surv",
      vital_rate == "number_seeds"  ~ "seed",
      ...
      parameter == "intercept" ~ "int",
      parameter == "slope"     ~ "slope",
      .default = parameter),parameter = paste(vital_rate, parameter, sep = ".")) %>%
  dplyr::filter(parameter %in% c("growth.int","growth.slope","growth.sigma",
                                 "surv.int","surv.slope",
                                 "flow.int","flow.slope",
                                 "seed.int","seed.slope",
                                 "rcsz.r_mean","rcsz.r_sd","germ.int","est.int")) %>%
  # keep warm straight from ctrl_warm
  dplyr::mutate(
    parameter = dplyr::case_when(
      parameter == "rcsz.r_mean" ~ "r_mean",
      parameter == "rcsz.r_sd"   ~ "r_sd",
      .default = parameter),warm = ctrl_warm) %>%
  dplyr::select(site, species, warm, vital_rate, parameter, values_control)


# merge control values, merge sensitivities, compute contributions
vr_ltre_all1 <- vr_long_long_con1 %>%
  dplyr::left_join(vr_ltre_control_long1,
                   by = c("site","species","warm","vital_rate","parameter")) %>%
  dplyr::left_join(sensitivity_df_cfd1,
                   by = c("site","species","parameter","treat_combi")) %>%
  dplyr::mutate(values_org     = ifelse(is.na(values_org),     0, values_org),
                values_control = ifelse(is.na(values_control), 0, values_control),
                sensitivity    = ifelse(is.na(sensitivity),    0, sensitivity)) %>%
  dplyr::mutate(contri = sensitivity * (values_org - values_control))



# vital-rate contributions for plotting / reporting
vr_ltre_contribution1 <- vr_ltre_all1 %>%
  dplyr::group_by(species, site, treat_combi, vital_rate) %>%
  dplyr::summarise(sum_contri = sum(contri, na.rm = TRUE), .groups = "drop") %>%
  dplyr::mutate(site_treat = paste(site, treat_combi, sep = "."))
...
# helpwer to parse a "species_site_treat" key into parts
parse_key <- function(key){
  x <- strsplit(key, "_", fixed = TRUE)[[1]]
  list(species = x[1], site = x[2], treat = x[3])
}

# recompute direct lambda (treat & control) on midpoint L/U for ALL pairs in the competition LTRE
lambda_direct_common1 <- lapply(names(params_list_ltre_con1), function(key_mid){
  
  # key_mid is like "scacol_hi_vege.warm"
  parts <- parse_key(key_mid)
  species <- parts$species; site <- parts$site; treat <- parts$treat
  
  # derive warm from treat (vege.ambi / vege.warm)
  warm <- sub("^[^.]+\\.", "", treat) # "ambi" / "warm"
  
  # treatment key (original parameters come from params_list)
  treat_key   <- paste(species, site, paste0("vege.", warm), sep = "_")
  # matching control key
  control_key <- paste(species, site, paste0("bare.", warm), sep = "_")
  
  # if either is missing in the original param list, skip
  if(!(treat_key %in% names(params_list)) || !(control_key %in% names(params_list))) {
    return(NULL)
  }
  
  # fetch midpoint params (to get common L/U)
  par_m <- params_list_ltre_con1[[key_mid]]
  
  # copy ORIGINAL parameters for treat/control
  par_t <- params_list[[treat_key]]
  par_c <- params_list[[control_key]]
  ...
  # recompute lambda on common bounds
  lam_t <- lambda.iter(buildK(par_t, n = 500))$lambda
  lam_c <- lambda.iter(buildK(par_c, n = 500))$lambda
  
  # return a small data.frame row
  data.frame(
    species = species,
    site = site,
    warm = warm,
    ...
    lambda_treat_common = lam_t,
    lambda_control_common = lam_c,
    delta_lambda_direct_common = lam_t - lam_c,
    stringsAsFactors = FALSE
  )
})

# bind into one table
lambda_direct_common1 <- dplyr::bind_rows(lambda_direct_common1) %>%
  mutate(treat_combi = str_sub(treat_key, 11, 19),
         site_treat = paste(site, treat_combi, sep = "."))
...
# sum up all vital rate contributions
ltre_sum1 <- vr_ltre_all1 %>%
  group_by(site, species, treat_combi, treat_warm, treat_comp) %>%
  summarize(delta_lambda_LTRE = sum(contri, na.rm = TRUE)) %>%
  ungroup()

# compare signs match:
ltre_check1 <- lambda_direct_common1 %>%
  left_join(ltre_sum1, by = c("species","site","treat_combi")) %>%
  mutate(diff = delta_lambda_LTRE - delta_lambda_direct_common)

# flag any big discrepancies
bad1 <- ltre_check1 %>% filter(abs(diff) > 1e-2)

# plot 
ggplot() + 
  geom_bar(data = vr_ltre_contribution1, aes(x = species, y = sum_contri, fill = vital_rate), position = "stack", stat = "identity") +
  geom_point(data = lambda_direct_common1, aes(x = species, y = delta_lambda_direct_common)) +
  facet_wrap(~site_treat) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank()) +
...
# comparisons made & corresponding controls:
# 1) bare/vege lo vs. bare/vege hi warm: site, but "no" temperature effect --> control: bare/vege lo, group by species, vital rate and treat_comp
# 2) bare/vege lo vs. bare/vege hi ambi: site & temperature effect --> control bare/vege lo, group by species, vital rate and treat_comp
# 3) bare vs. vege for each lo, hi ambi and hi bare: competition effect --> bare lo/hi ambi/hi warm

# 1) define control
vr_ltre_control_1 <- vr_long %>%
  filter(site == "lo" & treat_combi %in% c("bare.ambi", "vege.ambi")) 

# 2) define control
vr_ltre_control_2 <- vr_long %>%
  filter(site == "lo" & treat_combi %in% c("bare.ambi", "vege.ambi")) 


# 3) define control
vr_ltre_control_3 <- vr_long %>%
  filter(treat_comp == "bare") 
...
# define the three treatment data frames
# 1)
vr_ltre_treat_1 <- vr_long %>%
  filter(site == "hi" & treat_warm == "warm")

# 2) 
vr_ltre_treat_2 <- vr_long %>%
  filter(site == "hi" & treat_warm == "ambi") 

# 3) 
vr_ltre_treat_3 <- vr_long %>%
  filter(treat_comp == "vege") 


# 1) calculate midpoints
# create an empty data frame to store the midpoints
midpoints_1 <- data.frame()

# loop through each row in vr_long
for (i in 1:nrow(vr_ltre_treat_1)) {
  
  # get species and site for the current row in vr_long
  species_curr <- vr_ltre_treat_1$species[i]
  treat_comp_curr <- vr_ltre_treat_1$treat_comp[i]
  vital_rate_curr <- vr_ltre_treat_1$vital_rate[i]
  
  # get the corresponding control row in vr_ltre_control for the same species
  control_row <- vr_ltre_control_1 %>%
    filter(species == species_curr, treat_comp == treat_comp_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)
  
  # extract the vital rate value for the current treatment combination
  treatment_row <- vr_ltre_treat_1 %>%
    filter(species == species_curr, treat_comp == treat_comp_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)  
  
  # calculate midpoints for each parameter
  midpoint_values <- (treatment_row + control_row) / 2  # calculate midpoints
  
  # add these values to the new data frame
  midpoints_1 <- rbind(midpoints_1, cbind(species = species_curr, treat_comp = vr_ltre_treat_1$treat_comp[i], treat_combi = vr_ltre_treat_1$treat_combi[i],
                                          site = vr_ltre_treat_1$site[i], vital_rate = vital_rate_curr, midpoint_values, comparison = "lo.ambi_hi.warm"))
}

# convert the result into a tibble or data frame with proper column names
midpoints_1 <- as.data.frame(midpoints_1)

# 2) calculate midpoints
# create an empty data frame to store the midpoints
midpoints_2 <- data.frame()

# loop through each row in vr_long
for (i in 1:nrow(vr_ltre_treat_2)) {
  
  # get species and site for the current row in vr_long
  species_curr <- vr_ltre_treat_2$species[i]
  treat_comp_curr <- vr_ltre_treat_2$treat_comp[i]
  vital_rate_curr <- vr_ltre_treat_2$vital_rate[i]
  
  # get the corresponding control row in vr_ltre_control for the same species
  control_row <- vr_ltre_control_2 %>%
    filter(species == species_curr, treat_comp == treat_comp_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)
  
  # extract the vital rate value for the current treatment combination
  treatment_row <- vr_ltre_treat_2 %>%
    filter(species == species_curr, treat_comp == treat_comp_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)  
  ...
  # add these values to the new data frame
  midpoints_2 <- rbind(midpoints_2, cbind(species = species_curr, treat_comp = vr_ltre_treat_1$treat_comp[i], treat_combi = vr_ltre_treat_2$treat_combi[i], 
                                          site = vr_ltre_treat_2$site[i], vital_rate = vital_rate_curr, midpoint_values, comparison = "lo.ambi_hi.ambi"))
}

# convert the result into a tibble or data frame with proper column names
midpoints_2 <- as.data.frame(midpoints_2)


# 3) calculate midpoints
# create an empty data frame to store the midpoints
midpoints_3 <- data.frame()

# loop through each row in vr_long
for (i in 1:nrow(vr_ltre_treat_3)) {
  
  # get species and site for the current row in vr_long
  species_curr <- vr_ltre_treat_3$species[i]
  site_curr <-vr_ltre_treat_3$site[i]
  treat_warm_curr <- vr_ltre_treat_3$treat_warm[i]
  vital_rate_curr <- vr_ltre_treat_3$vital_rate[i]
  
  # get the corresponding control row in vr_ltre_control for the same species
  control_row <- vr_ltre_control_3 %>%
    filter(species == species_curr, site == site_curr, treat_warm == treat_warm_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)
  
  # extract the vital rate value for the current treatment combination
  treatment_row <- vr_ltre_treat_3 %>%
    filter(species == species_curr, site == site_curr, treat_warm == treat_warm_curr, vital_rate == vital_rate_curr) %>%
    dplyr::select(slope, intercept, sigma, r_mean, r_sd)  
  ...
  # add these values to the new data frame
  midpoints_3 <- rbind(midpoints_3, cbind(species = species_curr, site = site_curr, treat_warm = vr_ltre_treat_1$treat_warm[i], treat_combi = vr_ltre_treat_3$treat_combi[i], 
                                          vital_rate = vital_rate_curr, midpoint_values, comparison = "bare_vege"))
}

# convert the result into a tibble or data frame with proper column names
midpoints_3 <- as.data.frame(midpoints_3)

# add U and L back on
vr_midpoints_1 <- left_join(midpoints_1, vr_long[, c(1:4, 15, 16)], by = c("site", "species", "treat_combi", "vital_rate")) # selected: the 4 merging variables plus L and U
vr_midpoints_2 <- left_join(midpoints_2, vr_long[, c(1:4, 15, 16)], by = c("site", "species", "treat_combi", "vital_rate")) # selected: the 4 merging variables plus L and U
vr_midpoints_3 <- left_join(midpoints_3, vr_long[, c(1:4, 15, 16)], by = c("site", "species", "treat_combi", "vital_rate")) # selected: the 4 merging variables plus L and U
...
# unique combinations of species, site, and treatment
sst_combinations1 <- vr_midpoints_1 %>%
  distinct(species, site, treat_combi)
sst_combinations2 <- vr_midpoints_2 %>%
  distinct(species, site, treat_combi)
sst_combinations3 <- vr_midpoints_3 %>%
  distinct(species, site, treat_combi)

### USING LO BARE AS CONTROL

# save all parameters for all combinations in a list
params_list_1 <- mk.params.list(data_vrcols = vr_midpoints_1, data_sstcombi = sst_combinations1)
params_list_2 <- mk.params.list(data_vrcols = vr_midpoints_2, data_sstcombi = sst_combinations2)
params_list_3 <- mk.params.list(data_vrcols = vr_midpoints_3, data_sstcombi = sst_combinations3)

# generate the kernels and plot for 1)
# list to store kernel results
ipm.midpoint_1 <- list()

# create a progress bar for kernel generation
pb2 <- progress_estimated(length(params_list_1))

# loop through the list of parameters to calculate IPM kernels and plot
for (name in names(params_list_1)) {
  ...
  # retrieve the current parameters, L and U from the list
  current_params <- params_list_1[[name]]
  L_current <- params_list_1[[name]]$L
  U_current <- params_list_1[[name]]$U
  
  # fit the kernel using the parameters
  ipm_kernel <- mk.kernel(n = 500, L = L_current, U = U_current, par = current_params, ceiling_growth = FALSE, ceiling_seeds = FALSE)
  
  # store the kernel and mesh points in the ipm.allcombis list
  ipm.midpoint_1[[name]] <- list(
    K = ipm_kernel$K,
    meshpts = exp(ipm_kernel$meshpts),
    L = L_current,
    U = U_current)
  ...
  species <- combi_parts[1]
  site <- combi_parts[2]
  treat_combi <- combi_parts[4] # "mid_" can be ignored
  
}

# calculate lambdas
lambda_results_midpoint_1 <- lapply(names(ipm.midpoint_1), function(name) {
  lambda.iter(k = ipm.midpoint_1[[name]]$K)  # apply lambda.iter to each K matrix while using the names of the elements for clarity
})

# name the output list
names(lambda_results_midpoint_1) <- names(ipm.midpoint_1)

# create a data frame for plotting
lambda_values_midpoint_1 <- sapply(lambda_results_midpoint_1, function(x) x$lambda)
names_list1 <- names(lambda_results_midpoint_1)
lambda.midpoint_1 <- data.frame(site_species_treat = names_list1, lambda = lambda_values_midpoint_1, stringsAsFactors = FALSE, row.names = NULL) %>%
  mutate(species = str_sub(site_species_treat, 1, 6),
         site = str_sub(site_species_treat, 8, 9),
         treat_combi = str_sub(site_species_treat, 11, 19),
         trend = ifelse(lambda < 1, "shrink", "grow"),
...
# generate the kernels and plot for 2)
# list to store kernel results
ipm.midpoint_2 <- list()

# create a progress bar for kernel generation
pb2 <- progress_estimated(length(params_list_2))

# loop through the list of parameters to calculate IPM kernels and plot
for (name in names(params_list_2)) {
  ...
  # retrieve the current parameters, L and U from the list
  current_params <- params_list_2[[name]]
  L_current <- params_list_2[[name]]$L
  U_current <- params_list_2[[name]]$U
  
  # fit the kernel using the parameters
  ipm_kernel <- mk.kernel(n = 500, L = L_current, U = U_current, par = current_params, ceiling_growth = FALSE, ceiling_seeds = FALSE)
  
  # store the kernel and mesh points in the ipm.allcombis list
  ipm.midpoint_2[[name]] <- list(
    K = ipm_kernel$K,
    meshpts = exp(ipm_kernel$meshpts),
    L = L_current,
    U = U_current)
...
# calculate lambdas
lambda_results_midpoint_2 <- lapply(names(ipm.midpoint_2), function(name) {
  lambda.iter(k = ipm.midpoint_2[[name]]$K)  # apply lambda.iter to each K matrix while using the names of the elements for clarity
})

# name the output list
names(lambda_results_midpoint_2) <- names(ipm.midpoint_2)

# create a data frame for plotting
lambda_values_midpoint_2 <- sapply(lambda_results_midpoint_2, function(x) x$lambda)
names_list2 <- names(lambda_results_midpoint_2)
lambda.midpoint_2 <- data.frame(site_species_treat = names_list2, lambda = lambda_values_midpoint_2, stringsAsFactors = FALSE, row.names = NULL) %>%
  mutate(species = str_sub(site_species_treat, 1, 6),
         site = str_sub(site_species_treat, 8, 9),
         treat_combi = str_sub(site_species_treat, 11, 19),
         trend = ifelse(lambda < 1, "shrink", "grow"),
...
# create a progress bar for kernel generation
pb2 <- progress_estimated(length(params_list_3))

# loop through the list of parameters to calculate IPM kernels and plot
for (name in names(params_list_3)) {
  ...
  # retrieve the current parameters, L and U from the list
  current_params <- params_list_3[[name]]
  L_current <- params_list_3[[name]]$L
  U_current <- params_list_3[[name]]$U
  
  # fit the kernel using the parameters
  ipm_kernel <- mk.kernel(n = 500, L = L_current, U = U_current, par = current_params, ceiling_growth = FALSE, ceiling_seeds = FALSE)
  
  # store the kernel and mesh points in the ipm.allcombis list
  ipm.midpoint_3[[name]] <- list(
    K = ipm_kernel$K,
    meshpts = exp(ipm_kernel$meshpts),
    L = L_current,
    U = U_current)
...
# calculate lambdas
lambda_results_midpoint_3 <- lapply(names(ipm.midpoint_3), function(name) {
  lambda.iter(k = ipm.midpoint_3[[name]]$K)  # apply lambda.iter to each K matrix while using the names of the elements for clarity
})

# name the output list
names(lambda_results_midpoint_3) <- names(ipm.midpoint_3)

# create a data frame for plotting
lambda_values_midpoint_3 <- sapply(lambda_results_midpoint_3, function(x) x$lambda)
names_list3 <- names(lambda_results_midpoint_3)
lambda.midpoint_3 <- data.frame(site_species_treat = names_list3, lambda = lambda_values_midpoint_3, stringsAsFactors = FALSE, row.names = NULL) %>%
  mutate(species = str_sub(site_species_treat, 1, 6),
         site = str_sub(site_species_treat, 8, 9),
         treat_combi = str_sub(site_species_treat, 11, 19),
         trend = ifelse(lambda < 1, "shrink", "grow"),
...
# 1)
# define the perturbation 
perturbation <- 0.001 # .Machine$double.eps^(1/3) (0.001)

# create a list to store the results
sensitivity_results_1 <- list()

# loop through each parameter set (i.e. each species * site * treatment combination)
for (combi in names(params_list_1)) {
  
  # extract parameters for the current combination
  current_params <- params_list_1[[combi]]
  
  # apply the sensitivity analysis to the current parameters
  sens_result <- sensitivity.analysis.finitediff(params = current_params, delta = perturbation, ceil_growth = TRUE, ceil_seeds = FALSE)
  
  # store the result in the list using the combination name as the key
  sensitivity_results_1[[combi]] <- sens_result
}

# now combine all results into a single data frame
sensitivity_df_1 <- do.call(rbind, lapply(names(sensitivity_results_1), function(combi) {
  
  res <- sensitivity_results_1[[combi]] # extract combination
  res$species_site_treatment <- combi # add name
  
  return(res)
}))


# take out U and L, make some columns
sensitivity_df_1 <- sensitivity_df_1 %>% 
  filter(!(parameter %in% c("U", "L"))) %>%
  mutate(split_col = species_site_treatment) %>%
  separate(split_col, into = c("species", "site", "treat_combi"), sep = "_") %>%
  mutate(treat_warm = str_sub(treat_combi, 6,9),
         treat_comp = str_sub(treat_combi, 1, 4))

# 2)
# create a list to store the results
sensitivity_results_2 <- list()

# loop through each parameter set (i.e. each species * site * treatment combination)
for (combi in names(params_list_2)) {
  
  # extract parameters for the current combination
  current_params <- params_list_2[[combi]]
  
  # apply the sensitivity analysis to the current parameters
  sens_result <- sensitivity.analysis.finitediff(params = current_params, delta = perturbation, ceil_growth = TRUE, ceil_seeds = FALSE)
  
  # store the result in the list using the combination name as the key
  sensitivity_results_2[[combi]] <- sens_result
}

# now combine all results into a single data frame
sensitivity_df_2 <- do.call(rbind, lapply(names(sensitivity_results_2), function(combi) {
  
  res <- sensitivity_results_2[[combi]] # extract combination
  res$species_site_treatment <- combi # add name
...
# take out U and L, make some columns
sensitivity_df_2 <- sensitivity_df_2 %>% 
  filter(!(parameter %in% c("U", "L"))) %>%
  mutate(split_col = species_site_treatment) %>%
  separate(split_col, into = c("species", "site", "treat_combi"), sep = "_") %>%
  mutate(treat_warm = str_sub(treat_combi, 6,9),
...
# loop through each parameter set (i.e. each species * site * treatment combination)
for (combi in names(params_list_3)) {
  
  # extract parameters for the current combination
  current_params <- params_list_3[[combi]]
  
  # apply the sensitivity analysis to the current parameters
  sens_result <- sensitivity.analysis.finitediff(params = current_params, delta = perturbation, ceil_growth = TRUE, ceil_seeds = FALSE)
  
  # store the result in the list using the combination name as the key
  sensitivity_results_3[[combi]] <- sens_result
}

# now combine all results into a single data frame
sensitivity_df_3 <- do.call(rbind, lapply(names(sensitivity_results_3), function(combi) {
  
  res <- sensitivity_results_3[[combi]] # extract combination
  res$species_site_treatment <- combi # add name
...
# take out U and L, make some columns
sensitivity_df_3 <- sensitivity_df_3 %>% 
  filter(!(parameter %in% c("U", "L"))) %>%
  mutate(split_col = species_site_treatment) %>%
  separate(split_col, into = c("species", "site", "treat_combi"), sep = "_") %>%
  mutate(treat_warm = str_sub(treat_combi, 6,9),
...
# 1)
vr_long_org_1 <- vr_long_long_con %>%
  filter(site == "hi" & treat_combi %in% c("bare.warm", "vege.warm"))
  
  
vr_ltre_control_long_1 <- vr_ltre_control_1 %>%
  pivot_longer(cols = c(6:12), names_to = "parameter", values_to = "values_control") %>%
  mutate(vital_rate = case_when(vital_rate == "size_t1_log" ~ "growth",
                                vital_rate == "flower_status" ~ "flow",
                                vital_rate == "survival" ~ "surv",
                                vital_rate == "number_seeds" ~ "seed",
...
# merge the control value to the original values and the sensitivities
vr_ltre_all_1 <- vr_ltre_control_long_1 %>%
  left_join(vr_long_org_1, by = c("species", "vital_rate", "parameter", "treat_comp")) %>%
  left_join(sensitivity_df_1, by = c("species", "parameter", "treat_comp")) %>%
  dplyr::select(-c(species_site_treatment, original_lambda)) %>%
  dplyr::select(-matches("\\."), -site, -treat_combi, -treat_warm)

# calculate contributions
vr_ltre_all_1 <- vr_ltre_all_1 %>%
  mutate(contri = sensitivity * (values_org - values_control)) 

vr_ltre_contribution_1 <- vr_ltre_all_1 %>%
  group_by(species, treat_comp, vital_rate) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup() 

vr_ltre_contribution_broad_1 <- vr_ltre_all_1%>%
  mutate(vital_rate_broad = if_else(vital_rate %in% c("est", "germ", "rcsz"), "est_total", vital_rate)) %>%
  group_by(species, treat_comp, vital_rate_broad) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup()

# try plotting
ggplot(vr_ltre_contribution_1, aes(x = vital_rate, y = sum_contri, fill = treat_comp)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ species, scales = "free") + 
  theme_bw() +  
  labs(title = "LTRE Contributions by Vital Rate (lo ambi vs. hi warm)",
       x = "Vital Rate",
...
# 2)
vr_long_org_2 <- vr_long_long_con %>%
  filter(site == "hi" & treat_combi %in% c("bare.ambi", "vege.ambi"))


vr_ltre_control_long_2 <- vr_ltre_control_2 %>%
  pivot_longer(cols = c(6:12), names_to = "parameter", values_to = "values_control") %>%
  mutate(vital_rate = case_when(vital_rate == "size_t1_log" ~ "growth",
                                vital_rate == "flower_status" ~ "flow",
                                vital_rate == "survival" ~ "surv",
...
# merge the control value to the original values and the sensitivities
vr_ltre_all_2 <- vr_ltre_control_long_2 %>%
  left_join(vr_long_org_2, by = c("species", "vital_rate", "parameter", "treat_comp")) %>%
  left_join(sensitivity_df_2, by = c("species", "parameter", "treat_comp")) %>%
  dplyr::select(-c(species_site_treatment, original_lambda)) %>%
  dplyr::select(-matches("\\."), -site, -treat_combi, -treat_warm)

# calculate contributions
vr_ltre_all_2 <- vr_ltre_all_2 %>%
  mutate(contri = sensitivity * (values_org - values_control)) 

vr_ltre_contribution_2 <- vr_ltre_all_2 %>%
  group_by(species, treat_comp, vital_rate) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup() 

vr_ltre_contribution_broad_2 <- vr_ltre_all_2 %>%
  mutate(vital_rate_broad = if_else(vital_rate %in% c("est", "germ", "rcsz"), "est_total", vital_rate)) %>%
  group_by(species, treat_comp, vital_rate_broad) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup()
...
# try plotting
ggplot(vr_ltre_contribution_2, aes(x = vital_rate, y = sum_contri, fill = treat_comp)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ species, scales = "free") + 
  theme_bw() +  
  labs(title = "LTRE Contributions by Vital Rate (lo ambi vs. hi ambi)",
       x = "Vital Rate",
       y = "Sum of Contributions",
       fill = "Treatment Combination") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
...
# 3)
vr_long_org_3 <- vr_long_long_con %>%
  filter(treat_comp == "vege")


vr_ltre_control_long_3 <- vr_ltre_control_3 %>%
  pivot_longer(cols = c(6:12), names_to = "parameter", values_to = "values_control") %>%
  mutate(vital_rate = case_when(vital_rate == "size_t1_log" ~ "growth",
                                vital_rate == "flower_status" ~ "flow",
                                vital_rate == "survival" ~ "surv",
...
# merge the control value to the original values and the sensitivities
vr_ltre_all_3 <- vr_ltre_control_long_3 %>%
  left_join(vr_long_org_3, by = c("species", "site", "treat_warm", "vital_rate", "parameter")) %>%
  left_join(sensitivity_df_3, by = c("species", "site", "treat_warm", "parameter")) %>%
  dplyr::select(-c(species_site_treatment, original_lambda)) %>%
  dplyr::select(-matches("\\."), -treat_comp, -treat_combi)

# calculate contributions
vr_ltre_all_3 <- vr_ltre_all_3 %>%
  mutate(contri = sensitivity * (values_org - values_control)) 

vr_ltre_contribution_3 <- vr_ltre_all_3 %>%
  group_by(species, site, treat_warm, vital_rate) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(site_treat_warm = paste(site, treat_warm, sep = "."))

vr_ltre_contribution_broad_3 <- vr_ltre_all_3 %>%
  mutate(vital_rate_broad = if_else(vital_rate %in% c("est", "germ", "rcsz"), "est_total", vital_rate)) %>%
  group_by(species, site, treat_warm, vital_rate_broad) %>%
  summarize(sum_contri = sum(contri, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(site_treat_warm = paste(site, treat_warm, sep = "."))

# try plotting
ggplot(vr_ltre_contribution_3, aes(x = vital_rate, y = sum_contri, fill = site_treat_warm)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ species, scales = "free") + 
  theme_bw() +  
  labs(title = "LTRE Contributions by Vital Rate (bare vs. vege)",
       x = "Vital Rate",
       y = "Sum of Contributions",
       fill = "Treatment Combination") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
...
#vr_ltre_contribution_1 <- vr_ltre_contribution_1 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, treat_comp) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values er species * site * treatment combi
#  #ungroup() %>%
#  mutate(normalize = pos_contri / pos_sum) %>% # normalize contributions
#  mutate(rel_contri = ifelse(sum_contri < 0, (normalize * -1), normalize)) %>% # adapt sign
#  ungroup() 
#vr_ltre_contribution_2 <- vr_ltre_contribution_2 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, treat_comp) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values er species * site * treatment combi
#  #ungroup() %>%
...
#  mutate(rel_contri = ifelse(sum_contri < 0, (normalize * -1), normalize)) %>% # adapt sign
#  ungroup()
#vr_ltre_contribution_3 <- vr_ltre_contribution_3 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, site, treat_warm) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values er species * site * treatment combi
#  #ungroup() %>%
...
#  ungroup()
#
## calculate relative contribution for broad vital rate classes
#vr_ltre_contribution_broad_1 <- vr_ltre_contribution_broad_1 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, treat_comp) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values per species * site * treatment combi
#  #ungroup() %>%
#  mutate(normalize = pos_contri / pos_sum) %>% # normalize contributions
#  mutate(rel_contri = ifelse(sum_contri < 0, (normalize * -1), normalize)) %>% # adapt sign
#  ungroup() 
#vr_ltre_contribution_broad_2 <- vr_ltre_contribution_broad_2 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, treat_comp) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values per species * site * treatment combi
#  #ungroup() %>%
...
#  mutate(rel_contri = ifelse(sum_contri < 0, (normalize * -1), normalize)) %>% # adapt sign
#  ungroup() 
#vr_ltre_contribution_broad_3 <- vr_ltre_contribution_broad_3 %>%
#  mutate(pos_contri = abs(sum_contri)) %>% # get absolute values
#  group_by(species, site, treat_warm) %>%
#  mutate(pos_sum = sum(pos_contri, na.rm = TRUE)) %>% # get sum of absolute values per species * site * treatment combi
#  #ungroup() %>%
...
#  mutate(rel_contri = ifelse(sum_contri < 0, (normalize * -1), normalize)) %>% # adapt sign
#  ungroup() %>%
#  mutate(site_treat_warm = factor(site_treat_warm, levels = c("lo.ambi", "hi.warm", "hi.ambi")))
#
#
## plot the normalized contribution
#
## 1)
#ggplot(vr_ltre_contribution_broad_1, aes(x = vital_rate_broad, y = rel_contri, fill = treat_comp)) +
#  #geom_bar(stat = "identity", position = "dodge") + 
#  #facet_wrap(~ species, scales = "free") + 
#  geom_boxplot() +
#  theme_bw() +  
...
#  x = 1:5,  
#  y = rep(0.8, 5),  
#  label = c("bare   vege", "bare   vege", "bare   vege", "bare   vege", "bare   vege"))
#
#png("/Users/eviseli/Desktop/FigLTRE_site_poster.png", width=16, height=10, units="cm", res=800)
#
#ggplot(vr_ltre_contribution_broad_2, aes(x = factor(vital_rate_broad, levels = c("surv", "growth", "flow", "seed", "est_total")), y = rel_contri, pattern = treat_comp)) +
#  geom_abline(intercept = 0, slope = 0, linetype = "dashed", colour = "gray", size = 1) +
#  geom_boxplot_pattern(fill = "gray", color = "black", pattern_colour = "white", pattern_fill = "white", pattern_angle = 45, pattern_density = 0.3, 
#                       position = position_dodge(width = 0.9),
#                       pattern_spacing = unit(4, "mm"), pattern_key_scale_factor = 0.6) +
#  #geom_text(data = label_data, aes(x = x, y = y, label = label), 
#  #          size = 5, color = "black", vjust = 26, hjust = "center") + 
#  scale_pattern_manual(values = c(bare = "stripe", vege = "none")) +
#  theme_bw() +  
#  labs(y = "Normalized vital rate contribution") +
...
#        axis.text.x = element_text(vjust = 9, face = "bold", size = 14),
#        axis.text.y = element_text(size = 14),
#        axis.title = element_text(size = 16),
#        legend.position = "none") +
#  scale_x_discrete(labels = c("survival", "growth", "flowering", "fecundity", "est.")) +
#  guides(pattern = FALSE, fill = guide_legend(override.aes = list(pattern = "none"))) +
#  geom_point(aes(x = vital_rate_broad, y = rel_contri, group = treat_comp), position = position_dodge(width = 0.9)) +
#  ylim(-0.8, 1)
#
#dev.off()
#
...
#  x = 1:5,  
#  y = rep(0.8, 5),  
#  label = c("lo.ambi hi.warm hi.ambi", "lo.ambi hi.warm hi.ambi", "lo.ambi hi.warm hi.ambi", "lo.ambi hi.warm hi.ambi", "lo.ambi hi.warm hi.ambi"))
#
#png("/Users/eviseli/Desktop/FigLTRE_comp_poster.png", width = 20, height=10, units="cm", res=800)
#
#ggplot(vr_ltre_contribution_broad_3, aes(x = factor(vital_rate_broad, levels = c("surv", "growth", "flow", "seed", "est_total")), y = rel_contri, fill = site_treat_warm)) +
#  geom_abline(intercept = 0, slope = 0, linetype = "dashed", colour = "gray", size = 1) +
...
#        axis.title = element_text(size = 16),
#        legend.position = "none") +
#  scale_fill_manual(name = "Temperature treatment", values = c("hi.ambi" = "#5f6632", "hi.warm"= "#a7b051", "lo.ambi" = "#fde861"), 
#                    labels = c("hi_warm" = "beyond: warmed", "hi_ambi"= "beyond: ambient", "lo_ambi" = "within: ambient")) +
#  scale_x_discrete(labels = c("survival", "growth", "flowering", "fecundity", "est.")) +
#  guides(pattern = FALSE, fill = guide_legend(override.aes = list(pattern = "none"))) +
#  geom_point(aes(x = vital_rate_broad, y = rel_contri, group = site_treat_warm), position = position_dodge(width = 0.9)) +
#  #geom_text(data = label_data2, aes(x = x, y = y, label = label, fill = "black"), 
#  #          size = 5, color = "black", vjust = 1, hjust = "center") +
#  ylim(-0.8, 1) 
#
#dev.off()
#
...
# get the differeces in lambda between the different controls

# 1)

lambda_diff1 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(site == "hi" & treat_combi %in% c("bare.warm", "vege.warm"))
lambda_control1 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(site == "lo") %>%
  dplyr::select(lambda, species, treat_comp) %>%
  rename("lambda_control" = "lambda")

# add together by species
lambda_diff1 <- lambda_diff1 %>%
  left_join(lambda_control1, by = c("species", "treat_comp")) %>%
  rowwise() %>%
  mutate(lambda_control = as.numeric(lambda_control),
         lambda = as.numeric(lambda),
         lambda_diff = lambda - lambda_control) %>%
  ungroup()

# 2)

lambda_diff2 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(site == "hi" & treat_combi %in% c("bare.ambi", "vege.ambi"))
lambda_control2 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(site == "lo") %>%
  dplyr::select(lambda, species, treat_comp) %>%
...
# add together by species
lambda_diff2 <- lambda_diff2 %>%
  left_join(lambda_control2, by = c("species", "treat_comp")) %>%
  rowwise() %>%
  mutate(lambda_control = as.numeric(lambda_control),
         lambda = as.numeric(lambda),
         lambda_diff = lambda - lambda_control) %>%
...
lambda_diff3 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(treat_combi %in% c("vege.warm", "vege.ambi"))
lambda_control3 <- lambda.allcombis %>%
  mutate(treat_comp = str_sub(treat_combi, 1, 4),
         treat_warm = str_sub(treat_combi, 6, 9)) %>%
  filter(treat_combi %in% c("bare.warm", "bare.ambi")) %>%
  dplyr::select(lambda, species, treat_warm, site) %>%
  rename("lambda_control" = "lambda")

# add together by species
lambda_diff3 <- lambda_diff3 %>%
  left_join(lambda_control3, by = c("species", "treat_warm", "site")) %>%
  rowwise() %>%
  mutate(lambda_control = as.numeric(lambda_control),
         lambda = as.numeric(lambda),
         lambda_diff = lambda - lambda_control) %>%
...
# add lambda differences to vital rate contributions

# 1)

vr_ltre_contribution_1_diff <- vr_ltre_contribution_1 %>%
  group_by(species, treat_comp) %>%
  summarize(tot_contribution = sum(sum_contri)) %>%
  left_join(lambda_diff1[, c(2, 3, 8, 11, 12)], by = c("species", "treat_comp")) # lambda, lambda_control, lambda_diff, treat:comp, species
  
# 2)

vr_ltre_contribution_2_diff <- vr_ltre_contribution_2 %>%
  group_by(species, treat_comp) %>%
  summarize(tot_contribution = sum(sum_contri)) %>%
  left_join(lambda_diff2[, c(2, 3, 8, 11, 12)], by = c("species", "treat_comp")) # lambda, lambda_control, lambda_diff, treat:comp, species

# 2)

vr_ltre_contribution_3_diff <- vr_ltre_contribution_3 %>%
  group_by(species, treat_warm, site) %>%
  summarize(tot_contribution = sum(sum_contri)) %>%
  left_join(lambda_diff3[, c(2:4, 7, 11:12)], by = c("species", "treat_warm", "site")) # lambda, lambda_control, lambda_diff, treat_warm, species, site

# plot them...

ggplot(data = vr_ltre_contribution_1_diff, aes(x = lambda_diff, y = tot_contribution)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1)
ggplot(data = vr_ltre_contribution_2_diff, aes(x = lambda_diff, y = tot_contribution)) +
  geom_point()  +
  geom_abline(intercept = 0, slope = 1)
ggplot(data = vr_ltre_contribution_3_diff, aes(x = lambda_diff, y = tot_contribution)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1)

ggplot() +
  geom_point(data = vr_ltre_contribution_1_diff, aes(x = lambda_diff, y = tot_contribution), col = "darkblue") +
  geom_point(data = vr_ltre_contribution_2_diff, aes(x = lambda_diff, y = tot_contribution), col = "darkred") +
  geom_point(data = vr_ltre_contribution_3_diff, aes(x = lambda_diff, y = tot_contribution), col = "orange") +
  theme_bw() +
  geom_abline(x = 0, y = 1, linetype = "dashed") +
  labs(x ="Difference in population growth rate", y = "Sum of vital rate contributions")

# especially the competition ones all sum up to higher values than the differences in lambda (this is a general trend though)

### plot stacks of contributions -----------------------------------------------

ggplot() + 
  geom_bar(data = vr_ltre_contribution_1, aes(x = species, y = sum_contri, fill = vital_rate), position = "stack", stat = "identity") +
  geom_point(data = vr_ltre_contribution_1_diff, aes(x = species, y = lambda_diff)) +
  facet_wrap(~treat_comp, labeller = as_labeller(c("bare" = "bare soil: w/o competition", "vege" = "vegetation: with competition"))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank()) +
  labs(y ="Vital rate contributions")
...
ggplot() + 
  geom_bar(data = vr_ltre_contribution_2, aes(x = species, y = sum_contri, fill = vital_rate), position = "stack", stat = "identity") +
  geom_point(data = vr_ltre_contribution_2_diff, aes(x = species, y = lambda_diff)) +
  facet_wrap(~treat_comp, labeller = as_labeller(c("bare" = "bare soil: w/o competition", "vege" = "vegetation: with competition"))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank()) +
...
ggplot() + 
  geom_bar(data = vr_ltre_contribution_3, aes(x = species, y = sum_contri, fill = vital_rate), position = "stack", stat = "identity") +
  geom_point(data = vr_ltre_contribution_3_diff, aes(x = species, y = lambda_diff)) +
  facet_wrap(site~treat_warm) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.x = element_blank()) +
  labs(y ="Vital rate contributions")

# looks better with new survival models, but probably not good enough

### bootstrapped contributions -------------------------------------------------


# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# VITAL RATE REPLACEMENTS  ----
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# code can be found on RangeX_IPM_FirstFit_20250924.R
